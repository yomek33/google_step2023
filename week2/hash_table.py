...
#     return hash
def calculate_hash(key):
    assert isinstance(key, str) #keyがstrかどうか
    hash = 0
    multiplier = 31 
    for i in key:
        hash = multiplier * hash + ord(i)
    return hash
...
class HashTable:
    ...
    def rehash_up(self): #テーブルサイズを二倍に拡張
        #テーブルのサイズが２倍の要素がテーブルを生成
    ...
    def rehash_down(self): #テーブルサイズを半分にする
        #テーブルのサイズが２倍の要素がテーブルを生成
        new_bucket_size = self.bucket_size // 2
        new_buckets = [None] * new_bucket_size
    ...
    def put(self, key, value):
        ...
        if self.item_count > self.bucket_size * 0.7:  # サイズ上限の70%を超えた場合
            self.rehash_up()
    ...
    def delete(self, key):
        ...
        #------------------------#
        bucket_index = calculate_hash(key) % self.bucket_size #bucketのindexを計算する
        item = self.buckets[bucket_index]
        item_prev = None
        while item:
            if item.key == key:
                if item_prev:
                    item_prev.next = item.next #直前のアイテムが次のアイテムを指す
                else: #リンクリストの先頭にある場合
