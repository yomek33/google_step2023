...
void my_add_to_free_list(my_metadata_t *metadata, int idx) {
...
void *my_malloc(size_t size) {
  ...
  my_metadata_t *metadata = my_heap[idx].free_head;
  ...
  while (metadata) {
    if (metadata->size >= size) {
      if (!best_metadata || (best_metadata && metadata->size < best_metadata->size)) {
  ...
  if (!metadata) {
    ...
    my_add_to_free_list(metadata, idx);
  ...
  my_remove_from_free_list(metadata, prev, idx);
  ...
  if (remaining_size > sizeof(my_metadata_t)) {
    ...
    my_add_to_free_list(new_metadata, idx);
  }
  return ptr;
}
...
void my_free(void *ptr) {
  ...
  int idx = my_get_bin_idx(metadata->size);
